...
describe("DELETE /api/orders/:id (Admin Only)", () => {
	...
	describe("should return 401, if", () => {
		...
		it("user is not authenticated (invalid token)", async () => {
			...
			const res = await deleteOrderRequest(order._id.toString(), invalidCookie);
	...
	describe("should return 400, if", () => {
		it("order ID is not a valid ObjectId", async () => {
			const res = await deleteOrderRequest(getInvalidId(), adminCookie);
	...
	describe("should return 403, if", () => {
		it("regular user tries to access admin endpoint", async () => {
			const res = await deleteOrderRequest(order._id.toString(), userCookie);
	...
	describe("should return 404, if", () => {
		it("order does not exist", async () => {
			...
			const res = await deleteOrderRequest(nonExistentOrderId, adminCookie);
	...
	describe("should return 204, if", () => {
		it("admin successfully deletes order", async () => {
			const res = await deleteOrderRequest(order._id.toString(), adminCookie);
		...
		it("admin can delete multiple orders", async () => {
			...
			// Delete first order
			const res1 = await deleteOrderRequest(order._id.toString(), adminCookie);
			expect(res1.status).toBe(204);

			// Delete second order
			const res2 = await deleteOrderRequest(order2._id.toString(), adminCookie);
			expect(res2.status).toBe(204);

			// Delete third order
			const res3 = await deleteOrderRequest(order3._id.toString(), adminCookie);
			expect(res3.status).toBe(204);
		});

		it("admin can delete orders with different statuses", async () => {
			// Create orders with different characteristics
			const paidOrder = await createTestOrder(user._id.toString(), undefined, {
				itemsPrice: 50000,
				totalPrice: 60000,
			});
		...
		it("admin can delete order immediately after creation", async () => {
			...
			const newOrder = await createTestOrder(user._id.toString());
			const res = await deleteOrderRequest(
