...
describe("POST /api/notifications", () => {
	...
	describe("should return 401, if", () => {
		...
		it("user is not authenticated (invalid token)", async () => {
			...
			const notificationData = getValidNotificationData(user._id);
			const res = await createNotificationRequest(notificationData, invalidCookie);
	...
	describe("should return 400, if", () => {
		it.each(getInvalidNotificationData())("$testCase", async ({ data, expectedError }) => {
	...
	describe("should return 201, if", () => {
		...
		it("notification is created with all valid notification types", async () => {
			...
			for (const type of types) {
				const notificationData = { ...getValidNotificationData(user._id), type };
				const res = await createNotificationRequest(notificationData, adminCookie);
		...
		it("notification is created with optional data field", async () => {
			...
			const notificationData = { ...getValidNotificationData(user._id), data: customData };
			const res = await createNotificationRequest(notificationData, adminCookie);
		...
		it("notification is created without optional data field", async () => {
			const { data, ...notificationDataWithoutData } = getValidNotificationData(user._id);
			const res = await createNotificationRequest(notificationDataWithoutData, adminCookie);
		...
		it("admin can create notifications for different users", async () => {
			...
			const anotherUserNotification = getValidNotificationData(anotherTestUser.user._id);

			const userRes = await createNotificationRequest(userNotification, adminCookie);
			const anotherUserRes = await createNotificationRequest(anotherUserNotification, adminCookie);
